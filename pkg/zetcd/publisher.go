...
// Delimiter is the delimiter used in etcd keys.
const Delimiter = '/'
...
func NewPublisher(cli EtcdClientInterface, opts ...PubOption) *Publisher {
	publisher := &Publisher{
		client:     cli,
...
func (p *Publisher) doRegister() (EtcdClientInterface, error) {
	cli := p.client
	if cli == nil {
		return nil, errors.New("client is nil")
	}

	lease, err := p.register(cli)
	if err != nil {
		return nil, err
	}
	p.lease = lease
...
func (p *Publisher) keepAliveAsync(cli EtcdClientInterface) error {
...
func (p *Publisher) register(client EtcdClientInterface) (clientv3.LeaseID, error) {
...
func (p *Publisher) revoke(cli EtcdClientInterface) {
